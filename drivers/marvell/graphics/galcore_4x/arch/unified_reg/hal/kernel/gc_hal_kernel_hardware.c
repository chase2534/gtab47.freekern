...
#if MRVL_PULSE_EATER_COUNT_NUM
    if((flag & gcvPOWER_FLAG_CLOCK_OFF) && Hardware->core == gcvCORE_MAJOR)
    {
...
#if MRVL_PULSE_EATER_COUNT_NUM
    if((flag & gcvPOWER_FLAG_CLOCK_ON) && Hardware->core == gcvCORE_MAJOR)
    {
        gcmkONERROR(
    ...
    )
{
    ...
    gcmkPRINT_N(12, "GPU[%d](ChipModel=0x%x ChipRevision=0x%x PowerStates=%d):\n",
                core,
                Hardware->identity.chipModel,
                Hardware->identity.chipRevision,
                Hardware->powerState);
